...
type Rule struct {
        ...
        // Phase 12d: DnsType filter — $dnstype=A or $dnstype=~AAAA
        // Empty = all types. "~A" = all except A.
        DnsType  string
        DnsTypeExclude bool
...
func parseOptions(r *Rule, options string) error {
        for _, opt := range strings.Split(options, ",") {
                ...
                // Phase 12d: $dnstype=A, $dnstype=AAAA, $dnstype=~A
                case strings.HasPrefix(opt, "dnstype="):
                        val := opt[len("dnstype="):]
                        if strings.HasPrefix(val, "~") {
                                r.DnsTypeExclude = true
                                r.DnsType = val[1:]
                        } else {
                                r.DnsType = val
                        }
